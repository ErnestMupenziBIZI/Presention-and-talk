...
###METHOD###    "Berger and Boos with sample size n=50 with confidence =99%
##################################################################################
##################################################################################
...
#Family wise error rate for four different estimated correlation n=50, confidence=99%
...
rhos <- seq(from=-1, to=1, by = 0.1)
alpa<-c( 0.0253275,0.025355,  0.0253275, 0.02532735)
As <-rhos
for (iiv in 1:length(alpa)){
  for ( ii in 1:length(rhos)){
    As[ii] <- looptest(loops=1000000,nn=100,mu=c(0,0),sigma1=1,sigma2=1,rho=rhos[ii],alp=alpa[iiv])[3] 
  }
  if(alpa[iiv] ==  0.0253275){
    As1 <- As 
  }
  if(alpa[iiv] == 0.025355){
    As2 <- As 
  }
  if(alpa[iiv] ==  0.0253275){
    As3 <- As 
  }
  if(alpa[iiv] == 0.02532735){
    As4 <- As 
  }
}
matplot(rhos,cbind(As1,As2,As3,As4), type="l",xlab = "ρ",ylab = "Family wise error rate ",xlim=c(-1,0.2), ylim=c(0.047, 0.051))
abline(h =0.05, untf = FALSE,col=4,lty=2)
abline(v = 1, untf = FALSE,col=5,lty=2)
abline(v = 0.00208, untf = FALSE,col=1,lty=2)
abline(v = 0.02452, untf = FALSE,col=2,lty=2)
abline(v = 0.00208, untf = FALSE,col=3,lty=2)
abline(v = 0.00204, untf = FALSE,col=4,lty=2)
abline(v = -1, untf = FALSE,col=5,lty=2)
