...
###Family wise error rate for four different estimated correlation n=50
###correlation (rpool=0.09125962, rblind=0.08387398, rfisher=0.09126035, rOP=0.09222236)
...
rhos <- seq(from=-1, to=1, by = 0.1)
alpa<-c(0.02546,0.02545, 0.025466, 0.025468) #hAlfs of max FWERs
As <-rhos
for (iiv in 1:length(alpa)){
  ...
  if(alpa[iiv] == 0.025466){
    As3 <- As 
  }
  if(alpa[iiv] == 0.025468){
    As4 <- As 
  }
}
...
abline(v = 0.01273, untf = FALSE,col=1,lty=2)
abline(v = 0.012725, untf = FALSE,col=2,lty=2)
abline(v = 0.012733, untf = FALSE,col=3,lty=2)
abline(v = 0.012734, untf = FALSE,col=4,lty=2)
abline(v = -1, untf = FALSE,col=5,lty=2)
legend ("bottomleft",c("r_pool","r_blind","r_fisher","r_OP"),lwd=c(1,1),lty=1:2,col=c("black","red","green","blue"),cex =0.57)
