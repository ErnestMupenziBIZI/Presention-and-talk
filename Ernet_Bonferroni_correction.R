...
library(MASS)
#bivariate normal distriuted data
...
looptest <- function(loops=1000000,nn=100,mu=c(0,0),sigma1=1,
          sigma2=1,rho=0.8,alp=0.05)
{
  restest <- matrix(0,loops,3)
  for(ii in 1:loops)
    restest[ii,] <- biv.ttest(nn=nn,mu=mu,sigma1=sigma1,
                  sigma2=sigma2,rho=rho,alp=alp)    
  c(mean(restest[,1]),mean(restest[,2]),(mean(restest[,3])))
}
##incorrect family wise error rate plotting 

looptest(loops=1000000,nn=100,mu=c(0,0),sigma1=1,
         sigma2=1,rho=0.8,alp=0.05)

rhos <- seq(from=-1, to=1, by = 0.1)
As <-rhos
for ( ii in 1:length(rhos)){
  As[ii] <- looptest(loops=1000000,nn=100,mu=c(0,0),sigma1=1,sigma2=1,
          rho=rhos[ii],alp=0.05)[3]
}
plot(rhos,As, type='l',main = "FWER against Correlation rho unadjusted",
     xlab = "ρ",ylab = "Family ise error rate")
